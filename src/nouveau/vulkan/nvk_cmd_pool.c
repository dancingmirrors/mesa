...
void
nvk_cmd_mem_cache_evict_all(struct nvk_device *dev,
                            struct nvk_cmd_mem_cache *cache)
{
   ...
   list_splicetail(&cache->free_mem, &dead);
   list_splicetail(&cache->free_gart_mem, &dead);
   list_inithead(&cache->free_mem);
   list_inithead(&cache->free_gart_mem);
   cache->mem_count = 0;
   cache->gart_mem_count = 0;
   ...
   list_for_each_entry_safe(struct nvk_cmd_mem, mem, &dead, link)
...
nvk_cmd_mem_cache_finish(struct nvk_device *dev,
                         struct nvk_cmd_mem_cache *cache)
{
   nvk_cmd_mem_cache_evict_all(dev, cache);
...
nvk_cmd_pool_destroy_mem(struct nvk_cmd_pool *pool, bool recycle)
{
   ...
   if (unlikely(pdev->debug_flags & NVK_DEBUG_NO_CMD_MEM_CACHE))
      recycle = false;
                       ...
                       struct nvk_cmd_mem **mem_out)
{
   ...
   if (mem) {
      ...
      const bool use_cache =
         !(pdev->debug_flags & NVK_DEBUG_NO_CMD_MEM_CACHE);
      ...
      if (mem == NULL) {
         ...
         if (result != VK_SUCCESS && use_cache) {
            nvk_cmd_mem_cache_evict_all(dev, &dev->cmd_mem_cache);
            result = nvk_cmd_mem_create(dev, force_gart, &mem);
         }
         if (result != VK_SUCCESS)
            return result;
      }
...
VKAPI_ATTR void VKAPI_CALL
nvk_TrimCommandPool(VkDevice _device,
                    VkCommandPool commandPool,
